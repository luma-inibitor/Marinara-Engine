...
- Storage note: `staging` is storage format 7, `main` is 6, and the sharding migration is **one-way**. Never point an older build at a newer data directory, and never suggest doing so — current builds refuse to open newer data and the launcher blocks the downgrade, but pre-#4708 builds will happily overwrite it.
