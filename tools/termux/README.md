...
- `~/Marinara-fork` — this fork, branch `luma/staging`
- `~/Marinara-main` — stock upstream, branch `staging` (beta channel)
...
`marinara dev <owner/repo> <branch> [--copy-data]` spins up a **one-off** checkout of any repo/branch to test it — for example a PR branch or someone else's fork:

```sh
marinara dev luma-inibitor/Marinara-Engine patch/mari-unsandboxed-shell
marinara dev luma-inibitor/Marinara-Engine claude/chat-search-ui-ux-h6qgyu --copy-data
```
...
- **`--copy-data` snapshots your real data into the instance** — a `cp -a` of `~/marinara-data`, never a share. Use it to test a branch against your actual chats without risking them: the dev instance may migrate the store one-way (staging's shard migration is unreadable by older builds), and it only ever migrates the copy. Implies no fixture seeding. Re-runs keep the existing copy; delete `.copied-data` in the checkout to re-copy.
...
- Config: `MARINARA_DEV_ROOT` (default `~/marinara-dev`), `MARINARA_DEV_PORT` (default `7870`), `MARINARA_DEV_SEED` (default `1`), `MARINARA_DATA_DIR` (copy source), `MARINARA_STNOTES_DIR` (default `~/st-notes`), `MARINARA_STNOTES_REMOTE`.
