...
- **Staging rebase note:** the prompt-line commit conflicts with staging's Mari work in `workspace-agent.service.ts` (one bullet). Resolution: keep the `%%MARI_SHELL_SANDBOX_LINE%%` token and mirror staging's current tool list into `mariShellSandboxPromptLine()`. The CHANGELOG hunk was dropped to avoid a conflict on every sync. Since 2.4.3 the shell spawner is the generic `spawnWorkspaceSandboxedProcess({ executable, args })`, so the Termux shell resolution moved up into `spawnWorkspaceSandboxedShell()`; `mari db transform` also consumes the sandbox status, and the patch explicitly excludes the `unsandboxed` backend there so untrusted transform scripts keep requiring a real OS sandbox behind their own `MARI_DB_ALLOW_UNSAFE_TRANSFORMS` gate. On the 2.4.6 sync upstream rewrote the shell runner: commands now spawn `detached` and are killed as a process group (`killSandboxedProcessTree`, #5892), and every run is followed by a sensitive-file scan that reverts and stages unreviewed changes (#5786). The unsandboxed spawn is `detached` as well, so the group kill reaches its grandchildren, and the post-run scan applies to it unchanged. Upstream also added `writableWorkspace` and `allowChildProcesses` options to `spawnWorkspaceSandboxedProcess()`; the unsandboxed backend cannot enforce either, so it refuses a spawn that asks for a read-only workspace or no child processes instead of ignoring the request. Today only `mari db transform` asks, and it already excludes the unsandboxed backend.
...
- **Fourth commit:** keeps effect tuning through upstream's paused-resize respawn. Upstream's fix for the keyboard-resize bug (`314a71a58`, adapted from our retired `patch/weather-resize-repaint`) respawns every fallback particle when a deferred resize is applied on resume, and its call passes no tuning, so tuned sizes and speeds snapped back to stock until particles turned over. The commit also adapts upstream's `weather-effects-suspended-repaint` lane, which assumes the stock renderer: a silent `OffscreenCanvas` stub for the worker half, and for the fallback half a sandbox that holds the renderer module and the config/moon/tuning refs, with particles counted as `effectiveParticleCount()` plus fireflies. The lane now asserts that respawned weather particles keep the user's tuning.
...
### `patch/store-hash-join`

- **What:** Makes joined selects in the file-backed store index the joined table when the join condition carries an equality between a column of the joined table and a column already in the context (`equiJoinKey` / `joinCandidates` in `SelectQuery.run()`). The joined table is bucketed by that column once and probed per base row; the full join condition is still evaluated on every candidate, so results and their order are unchanged, and joins without such an equality keep the full scan.
- **Why forked:** Found live on the phone on 2026-09-22. `getPreviousOutput` (`agents.storage.ts`) joins a chat's agent runs against its messages during the agent phase of every generation, and the store ran that join as a nested loop with an object spread per row pair. On a chat with about 7,300 runs and 19,600 messages that is 142 million pairs: the event loop was blocked for 9.75 minutes (the freeze detector logged "suspended ~536 s"), nothing was served meanwhile, and the client showed a stalled agent and unreachable settings. This is the server-side half of upstream issue #6388.
- **Upstream status:** Not yet submitted (candidate). Generic store fix with no fork reference; the regression is the proof to attach.
- **Validation:** `node ./scripts/run-regressions.mjs --filter store-join-index` (5,000 runs joined against 20,000 messages: result set, order, and a 5 s bound; the nested loop took minutes at that size and the 30 s runner budget kills it), `pnpm --filter @marinara-engine/server lint`, `pnpm check:e2e-types`.
- **Touches:** `packages/server/src/db/file-backed-store.ts`, `scripts/regressions/store-join-index.regression.ts`.

## Retired patches

### `patch/weather-resize-repaint` — retired on the 2.4.6 sync

Upstream adopted it. `314a71a58` (Issue Sweep #5865) adapts our patch, linked from upstream issue #5814, and says so in the regression lane's header. The worker's `drawFrame()` no longer bails on `hidden`, and the main-thread fallback defers a resize while paused and applies it from `ensureFallbackRunning()`. Upstream added one thing: the fallback respawns its particles for the new box when it applies the deferred resize. Staging carries its own rewrite of `weather-effects-suspended-repaint.regression.ts` under the same name as ours, so the rebase conflicted on every file the patch touched and the branch was skipped to empty. That lane assumes the stock renderer; `patch/ambient-sky` now carries the changes it needs to pass on the fork.

The known trade-off recorded for our version applies to upstream's too: while paused, the fallback keeps its old backing-store size and CSS scales the frozen frame. This fix was still **not verified on a physical Android device** when it was retired.

### `patch/impersonate-copy-button` — retired on the 2.4.6 sync

Upstream merged it. `2b7949083 feat(client): copy the impersonation prompt from the stored-guidance modal` is our own commit, and the maintainer's follow-up `caeac2539 fix(client): harden stored guidance copy actions` is the same change as the branch's second commit. `GenerationReplayDetailsModal.tsx` on `staging` is byte-for-byte identical to the branch. Upstream then changed our regression lane once more in `fd6900f67 fix: escape regression regex inputs`. The branch rebased to empty on this sync.
