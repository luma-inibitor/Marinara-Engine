...
- **Known gap:** `ChatSettingsDrawer`'s and `GameSurface`'s floating panels hold their open state in component-local `useState`, so they are not layers yet — back still exits the app when only one of those is open. They can be adopted incrementally with `useBackDismiss(open, close)`; the coarse `announceChatFloatingUiDismiss()` broadcast is deliberately *not* used as a fallback, because it exposes no "is anything open" state to arm the sentinel from and would require keeping the sentinel armed permanently.
