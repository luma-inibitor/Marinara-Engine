...
Upstream adopted it the same day it was written. PR #6564 (`43f99a59e`, closes upstream #6562) adds the same `equiJoinKey` / `joinCandidates` equality-bucket lookup to `SelectQuery.run()`, credits our commit `c8623720f` by URL, and adds one guard we lacked: joining the same table twice keeps the scan path, so the earlier binding is not used as the probe key. Upstream's regression is broader than ours (20 chats, a real `getPreviousOutput` call, both equality directions, `.limit`, the rejoin case). The old commit stays reachable as tag `retired/store-hash-join`.
