...
- **What:** Keeps the roleplay weather layer painted when its canvas is resized while ambient rendering is suspended. `ChatRoleplaySurface` pauses the layer as soon as the mobile composer takes focus; the app's viewport meta uses `interactive-widget=resizes-content`, so on Android the software keyboard then shrinks the layout viewport and the canvas parent resizes. Assigning `canvas.width`/`canvas.height` clears the canvas, and neither render path repainted it, so the weather vanished for as long as the keyboard was up. In the worker, `drawFrame()` bailed on `hidden` — which defeated the repaint its own resize handler already performed for exactly this reason, and was redundant besides, since `scheduleFrame()` tests `hidden` twice before ever reaching it; the only calls it blocked were `init` and `resize`, both of which must paint, so it is simply removed. In the main-thread fallback the rAF loop is stopped while paused, so a resize there clears a canvas nothing will redraw; rather than make the draw callable out of band, the resize is deferred while paused and applied from `ensureFallbackRunning()`, which already runs at resume.
- **Why forked:** Reported from our own Android use — the weather disappeared whenever the keyboard opened. Nothing fork-specific: the defect is in stock `staging` and, verified against `upstream/staging`, is still unfixed there despite several mobile-keyboard fixes landing since our mirror.
- **Upstream status:** Not yet submitted (candidate). Written to be upstream-offerable: 14 changed source lines, no new dependencies, no behavior change while unpaused. Worth offering.
- **Ordering:** listed after `patch/ambient-sky` because both touch `WeatherEffects.tsx` and `weather-effects.worker.ts`. Applied last, a hunk that fails to carry forward is a loud cherry-pick conflict rather than a silent regression. It currently cherry-picks onto `patch/ambient-sky` **cleanly** — verified, with `pnpm check` and both lanes green on the result — so no resolution recipe is needed. `patch/ambient-sky` does not fix this itself: it kept both the `hidden` guard and the `drawFrame(now, false)` resize repaint verbatim.
- **Known trade-off:** while paused, the fallback keeps its old backing-store size, so CSS scales the frozen frame into the new box. Normally invisible and self-cancelling — the keyboard closing restores the original size, making the deferred resize a no-op. A long pause (a saved draft holds `weatherEffectsPaused` true) combined with an orientation change would leave the layer distorted until the composer unfocuses. The worker path — what Android Chrome actually runs — resizes and repaints immediately and has no such artifact.
- **Validation:** `pnpm check`, and `node ./scripts/run-regressions.mjs --filter scripts/regressions/weather-effects-suspended-repaint.regression.ts` (auto-discovered lane; no `package.json` change). Negative-tested by restoring the `hidden` guard and confirming the lane fails. Also reproduced in Chromium against the real esbuild-bundled worker with a real `OffscreenCanvas`: after suspend + resize the pre-fix canvas is empty and the fixed one still shows the frozen scene. **Not yet verified on a physical Android device** — the keyboard's viewport resize was emulated.
