...
// The renderer builds its own scratch canvases (star fields, cloud sprites) via
// OffscreenCanvas. Node has neither OffscreenCanvas nor document, so stand one
// up here — deliberately silent, so scratch draws never land in `calls`.
function createSilentContext() {
  const target: Record<string, unknown> = {};
  return new Proxy(target, {
    get(_source, property: string) {
      if (property in target) return target[property];
      return () => (property.startsWith("create") ? { addColorStop() {} } : undefined);
    },
    set(_source, property: string, value) {
      target[property] = value;
      return true;
...
assert.match(
  resizeBody[1],
  /document\.hidden \|\| pausedRef\.current/u,
  "the fallback resize handler must defer while paused — resizing clears a canvas the stopped rAF loop cannot redraw",
);
assert.match(
  fallbackSource,
  /if \(resizePending\) resize\(\);/u,
  "the deferred resize must be applied when rendering resumes",
);
